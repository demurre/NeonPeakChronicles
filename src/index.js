...
class MyGame extends Phaser.Scene {
    ...
    create() {
        ...
        const nameText = this.add.text(fullWidthScreen / 2, (fullHeightScreen - 100) / 2, 'name', { fontFamily: 'Interstatic', fontSize: '32px', fill: '#fff' }).setOrigin(0.5);
        const newGameButton = this.add.text(fullWidthScreen / 2, fullHeightScreen / 2, 'New Game', { fontFamily: 'Interstatic', fontSize: '32px', fill: '#fff' }).setOrigin(0.5);
    ...
    startGame() {
        ...
        const stageText = this.add.text(700, 10, 'Stage 1', { fontSize: '24px', fill: '#fff' }).setOrigin(1, 0);
    ...
    createBackground() {
        ...
        this.add.text(10, 10, 'name', { fontFamily: 'Interstatic', fontSize: '24px', fill: '#fff', align: 'left' }).setOrigin(0, 0);

        // Відображення етапу (замість "1" можна використовувати змінну або стан гри)
        const currentStage = 1;
        this.add.text(fullWidthScreen - 75, 10, `Stage ${currentStage}`, { fontFamily: 'Interstatic', fontSize: '24px', fill: '#fff', align: 'right' }).setOrigin(0.5, 0);
    }

    createMainCharacter() {
        const mainCharacter = this.add.image((fullWidthScreen / 10) * 2, (fullHeightScreen / 3) * 2, 'mainCharacter').setScale(0.5);
        // Налаштування головного персонажа
        const mainCharacterHPBar = this.add.image((fullWidthScreen / 9) * 2, (fullHeightScreen / 10) * 9 - mainCharacter.height / 2, 'hpbar40').setScale(0.5);
    ...
    createEnemies() {
        const enemy1 = this.add.image((fullWidthScreen / 10) * 7, (fullHeightScreen / 3) * 2, 'enemy1').setScale(0.5);
        const enemy2 = this.add.image((fullWidthScreen / 10) * 9, (fullHeightScreen / 3) * 2, 'enemy2').setScale(0.5);
        // Налаштування бічних персонажів
        const enemy1HPBar = this.add.image((fullWidthScreen / 10) * 7, (fullHeightScreen / 10) * 9 - enemy1.height / 2, 'hpbar40').setScale(0.5);
        enemy1HPBar.setOrigin(0.5);
        const enemy2HPBar = this.add.image((fullWidthScreen / 10) * 9, (fullHeightScreen / 10) * 9 - enemy2.height / 2, 'hpbar40').setScale(0.5);
    ...
    createCards() {
        const card1 = this.add.image((fullWidthScreen / 10) * 3, (fullHeightScreen / 14) * 13, 'card1').setScale(0.5).setInteractive();
        const card2 = this.add.image((fullWidthScreen / 10) * 4, (fullHeightScreen / 14) * 13, 'card2').setScale(0.5).setInteractive();
        const card3 = this.add.image((fullWidthScreen / 10) * 5, (fullHeightScreen / 14) * 13, 'card3').setScale(0.5).setInteractive();
        const card4 = this.add.image((fullWidthScreen / 10) * 6, (fullHeightScreen / 14) * 13, 'card4').setScale(0.5).setInteractive();
        const card5 = this.add.image((fullWidthScreen / 10) * 7, (fullHeightScreen / 14) * 13, 'card5').setScale(0.5).setInteractive();
    ...
    showEffectCircle(target, color) {
        const circle = this.add.circle(target.x, target.y, target.displayWidth / 2, color, 0.5); // Створення круга з відповідним кольором
